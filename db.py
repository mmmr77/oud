...
class DataBase(metaclass=Singleton):
    ...
    def find_user_by_id(self, id_: int) -> dict:
        return self._fetch_one(select(User.__table__).where(User.id == id_))

    def insert_user(self, *args) -> None:
        id_, first_name, last_name, username, creation_datetime = args
        statement = (insert(User)
                     .values(id=id_, first_name=first_name, last_name=last_name, username=username,
                             creation_datetime=creation_datetime)
